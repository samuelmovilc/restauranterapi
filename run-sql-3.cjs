...
(async () => {
  try {
    const [rows] = await db.query(\`
      SELECT 
          pd.nombre_producto AS Producto_Nombre_En_Pedido,
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    if (err) throw err;
    const ws = sftp.createWriteStream('/tmp/run-sql-3.js');
    ws.on('close', () => {
      conn.exec('export PATH=/root/.nvm/versions/node/v14.21.3/bin:$PATH && node /tmp/run-sql-3.js', (err2, stream) => {
        let out = '';
        stream.on('close', () => {
          console.log(out);
          conn.end();
