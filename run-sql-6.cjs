  ...
  try {
    const [rows] = await db.query(\`
      ...
      WHERE v.estado = 'ACEPTADA' 
        AND pd.nombre_producto LIKE '%granizado%'
    \`);
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    if (err) throw err;
    const ws = sftp.createWriteStream('/tmp/run-sql-6.js');
    ws.on('close', () => {
      conn.exec('export PATH=/root/.nvm/versions/node/v14.21.3/bin:$PATH && node /tmp/run-sql-6.js', (err2, stream) => {
        let out = '';
        stream.on('close', () => {
          console.log(out);
          conn.end();
